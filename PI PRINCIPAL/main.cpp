...
int main(){
    ...
    BITMAP* desistiuu = load_bitmap("desistiu.bmp",NULL);
    ...
    while(!key[KEY_ESC] && obj2.y != 20 && !key[KEY_K]){
		...
		draw_sprite(buffer, corona, obj1.x , obj1.y);
		draw_sprite(buffer, face, obj2.x, obj2.y);
		draw_sprite(buffer, coronaazul, obj3.x , obj3.y);
		draw_sprite(buffer, corona, obj4.x , obj4.y);
		draw_sprite(buffer, corona, obj5.x , obj5.y);
		draw_sprite(buffer, coronaazul, obj6.x , obj6.y);
		draw_sprite(buffer, coronaazul, obj7.x , obj7.y);
		...
		if(colidir(&obj1, &obj2, face, corona) || colidir(&obj3, &obj2, face, coronaazul) || colidir(&obj4, &obj2, face, corona) || colidir(&obj5, &obj2, face, corona) || colidir(&obj6, &obj2, face, coronaazul) || colidir(&obj7, &obj2, face, coronaazul) || colidir(&obj8, &obj2, face, coronaverde)){
				obj2.x = 500;
				obj2.y = 500;
			}
  		draw_sprite(screen, buffer, 0, 0);
